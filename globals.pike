...
class _TwitchIRC(mapping options) {
	...
	int(0..1) update_options(mapping opt) {
		...
		//If we were previously kicked, don't try to reactivate.
		if (options->outdated) return 1;
...
class irc_callback {
	...
	Concurrent.Future irc_connect(mapping options) {
		...
		options = (["module": this, "version": 12, "encrypt": 1]) | (options || ([]));
		if (!options->user) {
...
class http_websocket
{
	...
	mapping wscmd_do_the_thing(object channel, mapping(string:mixed) conn, mapping(string:mixed) msg) {
	*/
}
...
mapping all_twitch_scopes = ([
	...
	"user:read:chat": "Read and react to chat", "user:write:chat": "Regular messages and /me",
...
//Generate a TOTP for a given secret, at the current or given time
//totp("JBSWY3DPEHPK3PXP", 1775805584) --> "64843651"
//Normally omit the second parameter to use the current time.
//Should match the TOTP codes generated by any other tool.
