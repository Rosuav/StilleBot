...
const render_element = { //Matches _element_types (see Pike code)
	...
	twitchid: el => [ //extcall
		...
		LABEL([INPUT({type: "checkbox", name: "permitted_only", checked: !!el.permitted_only}),
			" Restrict to the permitted user (if applicable)"]), BR(),
		LABEL([INPUT({type: "checkbox", name: "require_follower", checked: !!el.require_follower}),
			" Restrict to users who are following you "]),
			follower_scopes && BUTTON({class: "twitchlogin", type: "button", "data-scope": follower_scopes}, "Grant permission"),
			BR(),
		LABEL([INPUT({type: "checkbox", name: "require_subscriber", checked: !!el.require_subscriber}),
			" Restrict to current channel subscribers "]),
			subscriber_scopes && BUTTON({class: "twitchlogin", type: "button", "data-scope": subscriber_scopes}, "Grant permission"),
			BR(),
