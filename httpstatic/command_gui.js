...
const types = {
	anchor_command: {
		...
		provides: {
			...
			"{@sub}": "1 if the command was triggered by a subscriber, 0 if not",
	...
	foreach_vars: {
		...
		params: [{attr: "mode", values: "foreach"},
			{attr: "variable", label: "Variable name"},],
	...
	conditional_string: {
		...
		params: [
			{attr: "conditional", label: "Condition type", values: ["string", "contains", "regexp"],
				selections: {string: "String equals", contains: "String includes", regexp: "Regular expression"}},
			{attr: "casefold", label: "Case insensitive", values: bool_attr},
	...
	randrot: {
		color: "#ee7777", children: ["message"], label: el => (
			el.mode === "rotate" ? "Rotate"
			: el.mode === "switch" ? (el.switchon ? "Switch on " + el.switchon : "Switch")
			: "Randomize"),
		params: [{attr: "mode", label: "Mode", values: ["random", "rotate", "switch"],
				selections: {random: "Random", rotate: "Rotate", switch: "Switch"}},
			{attr: "rotatename", label: "Tag (optional)", values: cooldown_name, //Reuses the cooldown_name handler to hide any autogenerated ones and per-user status
				visible: el => el.mode === "rotate"},
			{attr: "switchon", label: "Switch on", visible: el => el.mode === "switch"}],
		typedesc: el => [
			"Each time this is triggered, pick one child and show it. ",
			el.mode === "random" ? "It will be chosen at random."
			: el.mode === "rotate" ? ["Rotation can specify a",
				BR(), "synchronization tag so multiple commands can rotate together."]
			: el.mode === "switch" ? ["Switching on some",
				BR(), "value will select the Nth option after evaluating that number."]
			: "Choose from randomized, rotating, or switching.", //Won't normally be seen.
		],
	},
	weight: {
...
function update_conditional(el) {
	...
	//TODO: Migrate at least some of this to use a more general system (see randrot)
...
on("change", "#setprops select", e => {
	//Whenever you change any drop-down, check every attribute's visibility status.
	const type = types[propedit.type];
	const state = retrieve_attrs({}, type.params); //Not necessary if there are no visibility flags but w/e
	if (type.params) for (let param of type.params) {
		if (!param.attr || !param.visible) continue;
		const el = document.getElementById("value-" + param.attr);
...
function retrieve_attrs(dest, params) {
	for (let param of params) if (param.attr) {
		const val = param.attr !== "builtin" && document.getElementById("value-" + param.attr);
		if (val) {
			const values = param.values || default_handlers;
			let value = values.retrieve_value ? values.retrieve_value(val, dest) : val.value;
...
on("submit", "#setprops", e => {
	...
	const type = types[propedit.type];
	if (!propedit.template && type.params) retrieve_attrs(propedit, type.params);
