	...
	if (params->emotes)
	{
		...
		foreach (msg / " ", string word) {
			...
			if (cheerinfo && digits != "" && digits != "0" && blank == "") {
				...
				//other emotes.
				ret->emotes += ({({"/" + cheerinfo->prefix + "/" + digits, ofs, ofs + sizeof(word) - 1})});
			}
			ofs += sizeof(word) + 1;
		}
...
constant trig_sub = special_trigger("!sub", "Someone subscribes for the first time", "The subscriber", "tier, multimonth", "Stream support");
constant trig_resub = special_trigger("!resub", "Someone announces a resubscription", "The subscriber", "tier, months, streak, multimonth, msg", "Stream support");
...
constant trig_cheer = special_trigger("!cheer", "Any bits are cheered", "The cheerer", "bits, msg, msgid", "Stream support");
	...
	__async__ void _send_with_catch(mapping person, echoable_message message, mapping vars, mapping cfg) {
		if (mixed ex = catch {await(_send_recursive(person, message, vars, cfg));}) {
			...
			if (arrayp(ex) && sizeof(ex) && stringp(ex[0])) {
				...
				if (cfg->simulate) cfg->simulate("ERROR: " + String.trim(ex[0]));
				else report_error("ERROR", String.trim(ex[0]), "");
...
__async__ void http_handler(Protocols.HTTP.Server.Request req) {
	...
	sleep(1); //Delay queries (but count the time without this)
...
__async__ void conduit_message(Protocols.WebSocket.Frame frm, mapping conn) {
	...
	G->G->conduit_fallen_over = call_out(conduit_fallen_over, CONDUIT_KICK_TIMEOUT);
...
__async__ void establish_hook_notification(string|int channelid, string hook, mapping|void cond) {
	...
	else G_G_("eventhooks", hook, "")[(string)channelid] = 1; //Don't readd the same hook twice
}
...
__async__ void setup_http_server() {
	string http_address = G->G->instance_config->http_address;
	if (mixed ex = http_address && http_address != "" && catch {
		int use_https = has_prefix(http_address, "https://");
		string listen_addr = "::"; //By default, listen on IPv4 and IPv6
		int listen_port = use_https ? 443 : 80; //Default port from protocol
		sscanf(http_address, "http%*[s]://%*s:%d", listen_port); //If one is set for the dest addr, use that
		...
		{
			//FIXME: If no sugarmill keyfob available, report lack of SSL availability, don't stall out
			//(Allow an autogenerated? Or just reject SSL and insist on an unencrypted connection?)
			object pem = await(request_certificate("stillebot.com"));
			G->G->httpserver = Protocols.WebSocket.SSLPort(http_handler, ws_handler, listen_port, listen_addr,
				pem->get_private_key(), pem->get_certificates());
			register_ssl_certificate("stillebot.com", G->G->httpserver->ctx);
			...
			await(provide_ssl_certificate(G->G->instance_config->local_address, G->G->httpserver->ctx));
		}
	})
	{
