...
echoable_message process(object channel, object person, string param)
{
	...
	//may look up raids.
	if (!persist_config["ircsettings"]) return 0; //Bot not configured (but how are you triggering the command then??)
	string bot = persist_config["ircsettings"]->nick; if (!bot) return 0; //Bot not authenticated (ditto!)
