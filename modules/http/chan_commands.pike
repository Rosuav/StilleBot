...
void wscmd_update(object channel, mapping(string:mixed) conn, mapping(string:mixed) msg) {
	array valid = G->G->cmdmgr->validate_command(channel, (conn->group / "#")[0], msg->cmdname, msg->response, msg->original);
...
void websocket_cmd_validate(mapping(string:mixed) conn, mapping(string:mixed) msg) {
	...
	array valid = G->G->cmdmgr->validate_command(channel, mode, msg->cmdname || "validateme", msg->response, msg->original);
