...
__async__ mapping(string:mixed) find_channel(Protocols.HTTP.Server.Request req, string chan, string endpoint)
{
	...
	if (mapping user = req->misc->session->?user) {
		...
		else req->misc->chaninfo->save_or_login = "*You're logged in, but not a recognized mod.* [Show mod swords](:#recheck-modswords)";
		...
		if ((int)user->id == channel->userid) {
			...
			if (sizeof(need_scopes)) {
				...
				notification_banner = #{<div style="background: #fec; border: 1px solid rebeccapurple; position: fixed; left: 120px; right: 0px; top: 40px; padding: 0.25em; text-align: center">
					NOTE: Some functionality may be inoperative until the necessary permissions are granted. #} + btn + "</div>";
			}
		}
	}
	...
	req->misc->chaninfo->menunav = sprintf(
		...
		"<a href=%q target=_blank><img src=%q alt=\"Channel avatar\" title=%q></a></nav>%s",
...
mapping(string:mixed) your_channel(Protocols.HTTP.Server.Request req, string|void tail) {
	...
	string query = req->query; if (query != "") query = "?" + query;
	return redirect(sprintf("/channels/%s/%s%s", user, tail || "", query), 302); //Not a 301, since it depends on the user
}
